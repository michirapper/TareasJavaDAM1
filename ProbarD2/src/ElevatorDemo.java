...
/**
 * This program simulates an elevator panel that skips the 13th floor, checking
 * for input errors.
 */
public class ElevatorDemo {
	public static void main(String[] args) {
		...
		if (in.hasNextInt()) // check if the input is an integer. Hint: use the Scanner method hasNextInt.
		{
			// Now we know that the user entered an integer
			int floor = in.nextInt();
			if (floor == 13) {
				System.out.println("Error: There is no thirteenth floor.");
			} else if (0 >= floor || 18 < floor) {
